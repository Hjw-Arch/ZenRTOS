...
//这里我们仅以规则通道为例
//开启温度传感器通道																   
void interTemperatureInit(void)
{
...
static uint16_t interTemperGetAverage(uint8_t ch,uint8_t times)
{
	uint32_t temp_val = 0;
	uint8_t t;
	for(t = 0; t < times; t++) {
		temp_val += getADC(ch);	//获取通道转换值
		taskDelay(10);
	}
...
//得到温度值
//返回值:温度值(扩大了100倍,单位:℃.)
double interTemperGetTemperature(void)
{
	uint32_t adcx;
//	short result;
 	double temperate;
	adcx = interTemperGetAverage(ADC_Channel_16, 5);		// 读取通道16内部温度传感器通道,10次取平均
